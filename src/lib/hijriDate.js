...
/** @param {{ hm: number; hd: number; hy: number }} h */
export function formatHijriLabel(h) {
...
export function formatGregorianLabelSv(year, month, day) {
  return `${day} ${GREGORIAN_MONTHS_SV[month - 1] ?? month} ${year}`;
}

/**
 * hijri + gregorian in same payload (aaaaaaaaaaaaaaaaaaaaaaaa)
 * @param {Record<string, any>} row
 * @param {number} year
 * @param {number} month
 * @param {number} day
 */
export function enrichPrayerTimesCalendar(row, year, month, day) {
